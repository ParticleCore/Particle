...
(function particle() {
    ...
    styleSheet.textContent = [
        ...
        '    opacity: 0;\n',
        '}\n',
        '#movie_player:not(.ended-mode) video, #movie_player:not(.ended-mode) .html5-main-video, #movie_player:not(.ended-mode) .html5-video-content{\n',
        '   top: 50% !important;\n',
        ...
        '    content: "";\n',
        '    display: block;\n',
        '    padding-top: calc(56.25% + ' + ((get('VID_PLR_CTRL_VIS') > 0) ? 0 : 30) + 'px);\n',
        '}\n',
        '#player #movie_player{\n',
        '    bottom: 0 !important;\n',
        '    left: 0 !important;\n',
    ...
    function handleEvents(type, target, event, call, capture) {
        ...
        if (target.eventStock && target.eventStock[event] && target.eventStock[event][name]) {
            target.removeEventListener(event, target.eventStock[event][name].call, capture);
        }
        if (type === 'add') {
            ...
            if ((target + '').split('HTML').length > 1) {
                eventStock.push(target.eventStock[event][name]);
            }
        }
        console.info(eventStock);
    }
    function clearOrphans() {
        ...
        while (i) {
            i -= 1;
            if (!document.contains(eventStock[i].target)) {
                eventStock[i].target.remove();
                remove.push(i);
            }
        }
        i = remove.length;
    ...
    function argsCleaner(config) {
        ...
        if (config.args.video_id) {
            if ((get('VID_PLR_ADS') && (!get('VID_SUB_ADS') || (get('VID_SUB_ADS') && !config.args.subscribed))) || (audioMode && audioMode.classList.contains('active'))) {
    ...
    function scriptExit(event) {
        ...
        event = event.target.getAttribute('name');
        if (event) {
            if (event === 'www/base') {
    ...
    function thumbMod() {
        ...
        if ((get('BLK_ON') || get('GEN_SDBR_ON')) && (window.location.pathname === '/' || window.location.pathname === '/results' || window.location.pathname === '/watch' || window.location.pathname === '/feed/music' || window.location.pathname === '/feed/subscriptions')) {
    ...
    function generalChanges() {
        ...
        function linkIterator(link) {
            if (link !== 'length' && channelLink[link].href.split('/').length < 6 && get('GEN_CHN_DFLT_PAGE') !== 'default') {
                channelLink[link].href += '/' + get('GEN_CHN_DFLT_PAGE');
            }
        }
        if (get('GEN_YT_LOGO_LINK') && window.yt && window.yt.config_ && window.yt.config_.LOGGED_IN) {
            logo = document.getElementById('logo-container');
            if (logo && logo.href === window.location.origin + '/') {
                logo.href = '/feed/subscriptions';
            }
        }
        if (get('GEN_REM_APUN') && window.location.pathname === '/watch' && autoplaybar) {
